...
function calculateFinancialModel(assumptions: any) {
  ...
  // Calculate for each year from 2023 to endYear
  for (let year = 2023; year <= endYear; year++) {
    ...
    // Historical data for 2023 and 2024
    if (year === 2023) {
      yearAssumptions = {
        students: { french: 1100, ib: 200 },
        tuition: { french: 75000, ib: 90000 },
        other_income: 5000000,
        staff_cost_pct: 0.45,
        opex_pct: 0.22,
        others_expenses_pct: 0.04,
        rent_model: { type: 'fixed', amount: 7500000 },
        is_historical: true
      };
    } else if (year === 2024) {
      yearAssumptions = {
        students: { french: 1250, ib: 275 },
        tuition: { french: 78000, ib: 92000 },
        other_income: 5250000,
        staff_cost_pct: 0.46,
        opex_pct: 0.21,
        others_expenses_pct: 0.045,
        rent_model: { type: 'fixed', amount: 7600000 },
        is_historical: true
      };
    } else if (nearTerm[yearStr]) {
      ...
      if (prevYearData) {
        yearAssumptions = {
          ...
          others_expenses_pct: prevYearData.others_expenses_pct || 0.05,
    ...
    const totalOperatingExpenses = staffCosts + otherOpex + othersExpenses + rent;
    ...
    const accountsPayable = ((staffCosts + otherOpex + othersExpenses + rent) / 365) * apDays;
    const deferredIncome = totalRevenues * deferredPct;
    const inventory = ((staffCosts + otherOpex + othersExpenses + rent) / 365) * inventoryDays;
